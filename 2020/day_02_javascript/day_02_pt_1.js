...
while ((match = regex.exec(passwordsList))) {
  const expectedPattern = new RegExp(match[3], "g");
  const occurrenceCounter = (match[4].match(expectedPattern) || []).length;

  if (match[1] <= occurrenceCounter && occurrenceCounter <= match[2]) {
