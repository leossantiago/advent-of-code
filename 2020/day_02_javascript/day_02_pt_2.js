...
const regex = /(\d+)-(\d+) (\w): (\w+)/g;

let match;
let validPasswordCounter = 0;

while ((match = regex.exec(passwordsList))) {
  const firstPosition = match[1] - 1;
  const secondPosition = match[2] - 1;
  const requiredCharacter = match[3];
  const password = match[4];

  if (
    (password[firstPosition] === requiredCharacter ||
      password[secondPosition] === requiredCharacter) &&
...
console.log("Total valid passwords: ", validPasswordCounter);
